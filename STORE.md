# Texte si date pentru listarea Google Play

## Nume aplicatie
Creative Monkeyz Portal (neoficial)

> Recomandare: include cuvantul "neoficial" / "fan" in nume sau in primele randuri ale descrierii, ca sa eviti respingerea pentru impersonare.

## Descriere scurta (max 80 caractere)
Show-urile Creative Monkeyz, organizate: Robotzi, gaming, muzica si momente.

## Descriere completa (max 4000 caractere)
Creative Monkeyz Portal este un proiect-tribut neoficial, facut de un fan, care aduna intr-un singur loc, organizat ca un "Netflix", show-urile canalului Creative Monkeyz.

Rasfoieste pe categorii si sezoane: Robotzi (toate sezoanele, cu numele reale ale episoadelor), IOBAGG si zeci de gameplay-uri (Wolfenstein, Dying Light, Dead Space, Call of Duty, Battlefield si altele), 3lar, Piramida, Rendam, MiEZ, interviuri, conferinte, muzica si clipuri de pe stream.

Functii:
- Cautare rapida si rasfoire stil Netflix, cu poze de coperta
- Lista mea (favorite) si Continua vizionarea
- Marcaj pentru clipurile vazute
- Buton Random si Pick-ul zilei
- Rand Populare (dupa vizualizari) si sortare in categorii
- Auto-play pentru episodul urmator
- Durata afisata pe fiecare clip
- Culoare de accent reglabila
- Functioneaza si offline (interfata)

Videoclipurile ruleaza direct din playerul oficial YouTube, asa ca toate vizualizarile raman la creatori.

Disclaimer: aplicatie neoficiala, neafiliata cu Creative Monkeyz. Tot continutul apartine creatorilor de drept si este redat de pe YouTube.

LLTCM, Long Live The Creative Monkeyz.

## Alte campuri
- Categorie: Entertainment (Divertisment)
- Email de contact: (al tau)
- Politica de confidentialitate (URL): https://sorin2net.github.io/CM_Portal/privacy.html
- Icon (512x512): assets/icon-512.png
- Feature graphic (1024x500): assets/feature.jpg
- Capturi de ecran telefon: minim 2 (le facem din portal)
- Content rating: completezi chestionarul (probabil Teen, fiindca duce la continut YouTube)
- Data safety: "Nu se colecteaza date" (doar stocare locala pe dispozitiv)
